...
### - day46(220624, 금) 문자열 문제
- 문자찾기, 대소문자 변환
- <배운 점>
  - 문자열 문제 기본
- <소회>
  - 다시 시작하는 마음으로 기본부터 찬찬히

### - day45(220623, 목) : 이것이 취업을 위한 코딩 테스트다 with 파이썬, ch.03 그리디
- 그리디 알고리즘 예제2
- <배운 점>
  - 이중 리스트 데이터 입력
...
- <소회>
  ...
  - 여전히 '그리디 알고리즘'의 정체성을 잘 모르겠음... 
    - 그냥 정규화된 틀 없이 고민해서 푸는 문제로 이해하려고 함

### - day44(220622, 수) : 이것이 취업을 위한 코딩 테스트다 with 파이썬, ch.03 그리디
- 그리디 알고리즘 예제1
- <배운 점>
  - 무한 루프 안에 for문과  if문 활용
  - 문제를 접했을 때, 반복되는 수열의 형태를 파악하여 해결하는 방법
- <소회>
  - 이런 로직도 정형화된 '알고리즘'이라고 말할 수 있는가...? 아직도 의문;;;
  - 쉬운 난이도라는 데, 나에게는 아직까지 어려움

### - day43(220621, 화) : 이것이 취업을 위한 코딩 테스트다 with 파이썬, ch.03 그리디
- 그리디 알고리즘 학습
- <배운 점>
  - 그리디 개념 및 원리
- <소회>
  - 이런 로직도 정형화된 '알고리즘'이라고 말할 수 있는가...? 조금 의문
  - 실제 문제를 통해 감각을 익힐 필요가 있어 보임
  
### - day42(220620, 월) : 백준 11729_하노이의 탑 이동 순서
- 블로그 참조
- <배운 점>
  - 재귀 함수 활용법
- <소회>
  - 머리로는 이해가 되는데, 이걸 코드화하는 것이 어려움
  - 익숙함의 문제인지, 개념 부족의 문제인지 잘 모르겠음
- <참조>
